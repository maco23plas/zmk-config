...
### Zoom をできるだけ自動化する (この VPS モード限定)

Zoom の「サインイン必須」会議に入るには、Bot をブラウザで Zoom にログインさせておく。
ログイン状態は Google と同じブラウザプロファイルに保存されるので、GUI のある PC で一度:

```bash
npm run bot:login          # Google と Zoom の両方のタブが開く (zoom だけなら bot:login zoom)
# 開いたタブで Bot 用の無料 Zoom アカウントにログイン → ブラウザを閉じる
```

その後 `data/` (プロファイル入り) を VPS のボリュームにコピーする。以後 Zoom 会議には
ログイン済みで参加するので、名前入力やサインインの壁で止まらなくなる。

**待機室について (正直な話):** ゲストでも会員でも、ホストの待機室そのものを Bot が突破する
ことはできない (セキュリティ機構なので実装しない)。自分が主催する Zoom は次のどれかで自動化する:

- Zoom 設定で該当会議の **待機室をオフ** にする、または
- Bot の Zoom アカウントを **共同ホスト/代替ホスト** に指定しておく、または
- 待機室オンのままなら、Bot が入室をリクエストした時に **一度だけ「許可」を押す**
  (`ADMIT_TIMEOUT_MIN` 分待つので、会議開始後に押せば間に合う)

社外主催の Zoom は基本「許可を押す」運用になる (tl.dv も同じ)。

> ⚠️ 注意: ログイン済みプロファイルを別の場所 (別 IP) の VPS に移すと、Zoom/Google が
> 本人確認 (2段階認証など) を再度求めることがある。その場合は VPS 上に一時的に GUI
> (VNC 等) を立てて `bot:login` をやり直すか、ローカル PC 常駐運用に切り替える。
...
**Q. Zoom は?**
Zoom は Web クライアントで入る。カレンダーの `/j/` `/w/` `/wc/join/` 形式の URL を自動検知する
(個人ミーティングルーム `/my/...` は非対応)。VPS モードで Bot を Zoom にログインさせておくと
サインイン必須の会議にも入れる (上記「Zoom をできるだけ自動化する」参照)。待機室がある場合は
ホストの許可が必要 (待機室の突破はしない)。

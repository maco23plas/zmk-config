...
- 録音は成果物 (artifact) と Drive に保存。リポジトリ本体には議事録 (Markdown) だけが
  `minutemate-state` ブランチに蓄積される。
- タスク台帳・会議 DB も `minutemate-state` ブランチで引き継がれるので、
  タスクの持ち越し追跡・週次レポート・事前ブリーフも機能する (週次: 月曜 8:00 JST)。

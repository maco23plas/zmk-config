...
if (get('SELECT COUNT(*) c FROM webinars').c > 0) {
...
replaceChatScript(webinar.id, parseChatScriptText(
...
const soon = createSession({ webinarId: webinar.id, startAt: now + 10 * MINUTE, capacity: 0 }, now);
const later = createSession({ webinarId: webinar.id, startAt: now + 3 * HOUR + 5 * MINUTE, capacity: 30 }, now);

createRule({ webinarId: webinar.id, weekdays: '1,2,3,4,5', timeJst: '20:00', capacity: 30, horizonDays: 14 }, now);
const generated = generateSessionsFromRules(now);

...
export function register(router) {
  router.get('/', async () => {
    const now = clock.now();
    return html(indexPage(await listOpenSessions(now, 30), now));
  });

  router.get('/reserve', async (ctx) => {
    const now = clock.now();
    const session = await getSession(ctx.query.get('session') || '');
    if (!session) {
      return html(errorPage(404, '開催枠が見つかりません', 'すでに終了しているか、URLが正しくない可能性があります。'), 404);
    }
    return html(reservePage(session, { now }));
  });

  router.post('/reserve', async (ctx) => {
    const now = clock.now();
    const session = await getSession(ctx.form.session_id || '');
    if (!session) return html(errorPage(404, '開催枠が見つかりません'), 404);

    const values = {
      name: ctx.form.name || '', email: ctx.form.email || '',
      phone: ctx.form.phone || '', note: ctx.form.note || '', agree: ctx.form.agree === '1',
    };
    if (!values.agree) {
      return html(reservePage(session, { error: '公式LINEで受け取ることに同意のうえ、お進みください。', values, now }), 400);
    }
    ...
    try {
      ...
      if (err instanceof ReservationError) {
        return html(reservePage(session, { error: err.message, values, now }), 400);
      }
      log.error('予約の作成に失敗:', err?.stack || err);
      return html(reservePage(session, { error: '受付に失敗しました。時間をおいてお試しください。', values, now }), 500);
    }
    ...
    return redirect(`/thanks/${reservation.watch_token}`, 303);
  });

  router.get('/thanks/:token', async (ctx) => {
    const reservation = await getByWatchToken(ctx.params.token);
    if (!reservation) return html(errorPage(404, '予約が見つかりません'), 404);
    return html(thanksPage(reservation, clock.now()));
  });

  router.get('/r/:token', async (ctx) => {
    const reservation = await getByWatchToken(ctx.params.token);
    if (!reservation) return html(errorPage(404, '予約が見つかりません'), 404);
    const notice = ctx.query.get('canceled') === '1' ? 'ご予約をキャンセルしました。' : '';
    return html(managePage(reservation, clock.now(), { notice }));
  });

  router.post('/r/:token/cancel', async (ctx) => {
    const reservation = await getByWatchToken(ctx.params.token);
    if (!reservation) return html(errorPage(404, '予約が見つかりません'), 404);

...
## 8. オンライン説明会システム（`webinar/`）
公式LINEと連携した説明会の予約〜配信システムを `webinar/` に追加しました。
**アンタイ（給付金）とは別事業でも使える汎用の仕組み**で、現在の文言は
**ジュエリーのお仕事説明会（副業・代理店のご案内）**向けに設定しています。
サイト（`docs/`）とは独立したNode.jsアプリで、GitHub Pagesではなくサーバー上で動きます。

- **予約サイト**で日時を選ぶ →「LINEを開いて連携する」1タップで公式LINEと紐づけ
- **開催当日の3時間前に視聴リンクを自動送信**（前日・10分前のリマインドも送信）
- 視聴は**リンクを開くだけ**。Zoom不要・アプリ不要で、開始時刻になると自動再生（疑似ライブ配信）

**サーバー代は0円**です。Cloudflare Workers の無料プラン（1分ごとの自動実行つき）で動くように作ってあります。
セットアップ・LINE設定・運用手順は **`webinar/README.md`** を参照してください。

> 6章と同じ判断を持ち込んでいます。**架空の視聴者数と参加者を装ったコメントは既定でオフ**。
> さらに副業・代理店のご案内は規制が厳しいため、**収入額を一切書かない**構成にしています
> （「未経験から月◯万円」等は景表法・特商法に触れるおそれがあります）。
> 事業形態によっては特定商取引法の業務提供誘引販売取引／連鎖販売取引に該当します。
> 詳細は `webinar/README.md` の8章を参照してください。
